...
public class LibraryService extends Service<LibraryServiceConfiguration> {
	...
	libraryFrom=configuration.getFrom();
	System.out.println(libraryFrom);
	destination=queueName;
	destination1=topicName;
	log.debug("{} - Queue name is {}. Topic name is {}",
		configuration.getFrom(), queueName,
		topicName);
    ...
    public static void listener() throws JMSException, JSONException, MalformedURLException{
    	...
    	while(true) {
    		...
    		//Adding the received books to the HashMap
    		Book book=new Book();
    		Book book1=new Book();
    		...
    		//Changing the status to available for a previously lost book, if received
